...
<h2 id="create-repo" class="heading bgcolor-heading">リポジトリ作成</h2>
<h3 class="heading bottomline-heading">ローカルにリポジトリを新規作成する</h3>
<pre class="code-block"><code class="shell">$ cd [リポジトリのパス]
$ git init [リポジトリ名]</code></pre>
<h3 class="heading bottomline-heading">Webサーバ上にリポジトリを新規作成する</h3>
<pre class="code-block"><code class="shell">$ cd [リポジトリのパス]
$ git init --bare [リポジトリ名]
$ chown -R apache:apache [リポジトリ名]
$ cd [リポジトリ名]
$ git update-server-info</code></pre>
<h2 id="local-repo" class="heading bgcolor-heading">ローカルリポジトリに対する操作</h2>
<h3 class="heading bottomline-heading">変更履歴を1行で表示する</h3>
<pre class="code-block"><code class="shell">$ git log --oneline</code></pre>
<h3 class="heading bottomline-heading">ワークツリーとステージの差分を確認する</h3>
<pre class="code-block"><code class="shell">$ git diff [ファイルパス]</code></pre>
<ul>
  <li><code>[ファイルパス]</code>を省略すると、全てのファイルについて確認する。</li>
</ul>
<h3 class="heading bottomline-heading">ステージへの追加とコミットを同時に行う</h3>
<pre class="code-block"><code class="shell">$ git commit -am "[メッセージ]"</code></pre>
<h3 class="heading bottomline-heading">直前のコミットをやり直す</h3>
<pre class="code-block"><code class="shell">$ git commit --amend</code></pre>
<ul>
  <li>必ず<code>git push</code>実行前に実行すること。</li>
</ul>
<h3 class="heading bottomline-heading">直前のコミットメッセージを変更する</h3>
<pre class="code-block"><code class="shell">$ git commit --amend -m "[メッセージ]"</code></pre>
<ul>
  <li>必ず<code>git push</code>実行前に実行すること。</li>
</ul>
<h3 class="heading bottomline-heading">過去のコミットメッセージにおけるCOMMITER・AUTHOR・メールアドレスを変更する</h3>
...
<h3 class="heading bottomline-heading">ワークツリーとステージの変更を取り消す</h3>
<pre class="code-block"><code class="shell">$ git reset --hard HEAD [ファイルパス]</code></pre>
<ul>
  <li><code>[ファイルパス]</code>を省略すると、全てのファイルについて元に戻す。</li>
</ul>
<h2 id="remote-repo" class="heading bgcolor-heading">リモートリポジトリに対する操作</h2>
<h3 class="heading bottomline-heading">originのURLを確認する</h3>
<p>以下のどちらかのコマンドを実行する。</p>
<pre class="code-block"><code class="shell">$ git config -l</code></pre>
<pre class="code-block"><code class="shell">$ git remote -v</code></pre>
<h3 class="heading bottomline-heading">originのURLを変更する</h3>
<pre class="code-block"><code class="shell">$ git remote set-url origin [URL]</code></pre>
<h3 class="heading bottomline-heading"><code>git push</code>実行時に<code>origin master</code>を省略する</h3>
