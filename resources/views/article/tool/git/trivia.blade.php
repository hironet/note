...
<h3 class="heading desc">Webサーバ上にリポジトリを新規作成する</h3>
<pre class="code-block"><code class="shell">$ cd [リポジトリのパス]
...
<h3 class="heading desc">ワークツリーとステージの変更を取り消す</h3>
...
<h3 class="heading desc">originのURLを変更する</h3>
<pre class="code-block"><code class="shell">$ git remote set-url origin [URL]</code></pre>
<h3 class="heading desc"><code>git push</code>実行時に<code>origin master</code>を省略する</h3>
...
<h3 class="heading desc"><code>git push</code>実行時に認証情報（ユーザ名・パスワード）の入力を省略する</h3>
...
<h3 class="heading desc">保存された認証情報を変更する</h3>
...
<h3 class="heading desc">オレオレ証明書対策などのため、特定のGitリポジトリにおいて、SSH証明書のチェックを無効にする</h3>
