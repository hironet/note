...
<pre class="code-block"><code class="shell"># nmcli c modify [ネットワークインタフェース名] +ipv4.routes "[宛先ネットワークのアドレス]/[サブネットマスク] [ゲートウェイのIPアドレス]"</code></pre>
<h3 class="heading bottomline-heading">DNSサーバを設定する</h3>
...
<h3 class="heading bottomline-heading">IPアドレスを固定割り当てに設定する</h3>
<pre class="code-block"><code class="shell"># nmcli c modify [ネットワークインタフェース名] ipv4.method manual</code></pre>
<h3 class="heading bottomline-heading">ネットワークインタフェースを再起動する</h3>
<pre class="code-block"><code class="shell"># nmcli c down [ネットワークインタフェース名]
# nmcli c up [ネットワークインタフェース名]</code></pre>
<h2 id="yum" class="heading bgcolor-heading">パッケージの管理（Yum）</h2>
<h3 class="heading bottomline-heading">リポジトリ一覧を表示する</h3>
...
<h3 class="heading bottomline-heading">yumでDVDからパッケージをインストールする</h3>
...
<h3 class="heading bottomline-heading">SSH鍵ペアを作成し、SSH公開鍵を接続先ホストに登録する</h3>
<pre class="code-block"><code class="shell">$ ssh-keygen -t rsa
$ ssh-copy-id -i ~/.ssh/id_rsa.pub [ユーザ名]@[接続先ホスト]</code></pre>
<h3 class="heading bottomline-heading">SSH接続設定を保存しておき、接続コマンドを簡単にする</h3>
...
$ ssh [接続名]</code></pre>
<h3 class="heading bottomline-heading">cronで実行したコマンドの標準出力および標準エラー出力がメールで飛ばないようにする</h3>
<pre class="code-block"><code class="shell">$ crontab -l
[分] [時] [日] [月] [曜日] [コマンド] &gt; /dev/null 2&gt;&amp;1</code></pre>
<h3 class="heading bottomline-heading">フォーマット付きのdateコマンドをcronで実行する</h3>
<pre class="code-block"><code class="shell">$ crontab -l
