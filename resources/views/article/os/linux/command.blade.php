...
<h2 id="arping" class="title">arping</h2>
<h3 class="oper">同一セグメント上で、pingに応答しないノードの存在を確認する</h3>
<pre class="block"><code class="shell">$ arping [IPアドレス]</code></pre>
<h3 class="oper">IPアドレスの重複が発生していないかをGratuitous ARPで確認する</h3>
<pre class="block"><code class="shell">$ arping -D [IPアドレス]</code></pre>
<h3 class="oper">IPアドレスの変更をGratuitous ARPで他のノードに通知し、MACアドレステーブルおよびARPテーブルを更新させる</h3>
<pre class="block"><code class="shell">$ arping -A [IPアドレス]</code></pre>
<h2 id="awk" class="title">awk</h2>
<h3 class="oper">ファイルの2列目のみ表示する</h3>
<pre class="block"><code class="shell">$ cat [ファイルパス] | awk '{ print $2 }'</code></pre>
<h3 class="oper">ファイルの2列目の値が50以上の行のみ表示する</h3>
<pre class="block"><code class="shell">$ cat [ファイルパス] | awk '{ if ($2 >= 50) print $0 }'</code></pre>
<h3 class="oper">ファイルの各レコードが","で区切られており、5フィールド目の値が"ok"、かつ、6フィールド目の値が"dev"にマッチする行のみ表示する</h3>
<pre class="block"><code class="shell">$ cat [ファイルパス] | awk -F ',' '{ if ($5 == "ok" &amp;&amp; $6 ~ /dev/) print $0 }'</code></pre>
<h3 class="oper">ログファイルから特定の時間帯の行のみ表示する</h3>
<pre class="block"><code class="shell">$ awk -F - '"2018/07/20 03:30:00" &lt; $1 &amp;&amp; $1 &lt;= "2018/07/20 03:35:00"' [ファイルパス]</code></pre>
<h2 id="basename" class="title">basename</h2>
<h3 class="oper">ファイルパスからファイル名部分を取り出す</h3>
<pre class="block"><code class="shell">$ basename /var/log/messages
messages</code></pre>
<h2 id="chmod" class="title">chmod</h2>
<h3 class="oper">ディレクトリにSticky Bitを設定する</h3>
...
<h2 id="chown" class="title">chown</h2>
<h3 class="oper">シンボリックリンクの所有者・所有グループを変更する</h3>
<pre class="block"><code class="shell">$ chown -h [所有者]:[所有グループ] [ファイルパス]</code></pre>
<h2 id="curl" class="title">curl</h2>
<h3 class="oper">WebサーバからHTTPヘッダとデータを取得する</h3>
<pre class="block"><code class="shell">$ curl -i [URL]</code></pre>
<h3 class="oper">WebサーバからHTTPヘッダのみを取得する</h3>
<pre class="block"><code class="shell">$ curl -I [URL]</code></pre>
<h3 class="oper">SSLサーバ証明書に問題のあるWebサーバからデータを取得する</h3>
<pre class="block"><code class="shell">$ curl -k [URL]</code></pre>
<h2 id="dd" class="title">dd</h2>
<h3 class="oper">I/O負荷をかける</h3>
<pre class="block"><code class="shell">$ dd if=/dev/zero of=test bs=1M count=512 oflag=direct</code></pre>
<h2 id="dirname" class="title">dirname</h2>
<h3 class="oper">ファイルパスからディレクトリ部分を取り出す</h3>
<pre class="block"><code class="shell">$ dirname /var/log/messages
/var/log</code></pre>
<h2 id="dmidecode" class="title">dmidecode</h2>
<h3 class="oper">ハードウェア情報を表示する</h3>
<pre class="block"><code class="shell">$ dmidecode</code></pre>
<h2 id="echo" class="title">echo</h2>
<h3 class="oper">空ファイルを作成する</h3>
<pre class="block"><code class="shell">$ echo -n > [ファイルパス]</code></pre>
<h3 class="oper"><code>\n</code>の位置で改行する</h3>
<pre class="block"><code class="shell">$ echo -e 'hello\nworld'
hello
world</code></pre>
<h2 id="expr" class="title">expr</h2>
<h3 class="oper">四則演算をする</h3>
...
<h2 id="test" class="title">test</h2>
...
<h2 id="view" class="title">view</h2>
<h3 class="oper">viをreadonlyモードで起動する</h3>
<pre class="block"><code class="shell">$ view [ファイルパス]</code></pre>
<ul>
  <li><code>vi -R</code>と同じ。</li>
</ul>
<h2 id="yes" class="title">yes</h2>
<h3 class="oper">CPUに高い負荷をかける</h3>
