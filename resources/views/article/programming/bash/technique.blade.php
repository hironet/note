...
<h3 class="desc">コーディングルールを決める（以下は一例）</h3>
<ul>
...
<h3 class="desc">エラー処理をエレガントに行う</h3>
...
<h3 class="desc">ホスト名からドメイン部分を取り除く</h3>
...
<h3 class="desc">実行スクリプトのファイル名を除いた絶対パスを取得する</h3>
<pre class="block"><code class="bash">script_dir=$(cd $(dirname $0); pwd)</code></pre>
<h3 class="desc">関数の呼び出し元に文字列を返す</h3>
<ul>
...
<h3 class="desc">ファイルの末尾100行のみ残し、それ以前の行を削除する</h3>
...
<h3 class="desc">ファイルの内容をソートし、元のファイルに結果を出力する</h3>
...
<h3 class="desc">パスワード認証のSSHでコマンドを実行する</h3>
...
interact
"</code></pre>
<h3 class="desc">デーモンとして動作するプログラムを起動・停止する</h3>
...
<h3 class="desc">変数の値が空だった場合に値を設定する</h3>
