...
<pre class="block"><code>$ nmcli c modify [ネットワークインタフェース名] ipv4.gateway [デフォルトゲートウェイのIPアドレス]
</code></pre>
<p>スタティックルートを設定する。</p>
<pre class="block"><code>$ nmcli c modify [ネットワークインタフェース名] ipv4.routes "[宛先ネットワークのアドレス]/[サブネットマスク] [ゲートウェイのIPアドレス]"
</code></pre>
...
<pre class="block"><code>$ nmcli c modify [ネットワークインタフェース名] ipv4.dns [DNSサーバのIPアドレス]
</code></pre>
<p>IPアドレスを固定割り当てに設定する。</p>
<pre class="block"><code>$ nmcli c modify [ネットワークインタフェース名] ipv4.method manual
</code></pre>
...
<pre class="block"><code>$ mkdir /media/CentOS
...
<pre class="block"><code>$ repoquery --list [パッケージ名]
</code></pre>
<p>RPMパッケージをダウンロードする。</p>
<ul>
  <li>事前に、<code>yum-utils</code>パッケージをインストールしておく必要がある。</li>
</ul>
<pre class="block"><code>$ yumdownloader --resolve [パッケージ名]
</code></pre>
<h3 id="rpm" class="title">パッケージの管理（RPM）</h3>
<p>ファイルがどのパッケージに含まれるかを確認する。</p>
<pre class="block"><code>$ rpm -qf [ファイルパス]
</code></pre>
...
<pre class="block"><code>$ timedatectl status
</code></pre>
<p>タイムゾーンを<code>Asia/Tokyo (JST)</code>に設定する。</p>
<pre class="block"><code>$ timedatectl set-timezone Asia/Tokyo
</code></pre>
...
<pre class="block"><code>$ ssh-keygen -t rsa
$ ssh-copy-id -i ~/.ssh/id_rsa.pub [ユーザ名]@[接続先ホスト]
</code></pre>
<p>SSH接続設定を保存しておき、接続コマンドを簡単にする。</p>
<pre class="block"><code>$ cat ~/.ssh/config
Host [接続名]
HostName [ホスト名]
User [ユーザ名]
