...
<p>I/O負荷をかける。</p>
<pre class="block">
<code>$ dd if=/dev/zero of=test bs=1M count=512 oflag=direct
</code></pre>
<h3 id="dirname" class="title">dirname</h3>
<p>ファイルパスからディレクトリ部分を取り出す。</p>
<pre class="block">
<code>$ dirname /var/log/messages
/var/log
</code></pre>
...
<pre class="block">
<code>$ find [ディレクトリパス] \( -user root -o -group root \)
</code></pre>
...
<pre class="block">
<code>$ find [ディレクトリパス] -newermt 'YYYYMMDD HH:MM'
</code></pre>
<p>特定のディレクトリ（<code>/home</code>および<code>/proc</code>）配下を検索しないようにする。</p>
<pre class="block">
<code>$ find [ディレクトリパス] -not -path '/home/*' -not -path '/proc/*'
</code></pre>
<p>検索結果のファイル名だけ取り出す。</p>
<pre class="block">
<code>$ find [ディレクトリパス] -type f -exec basename {} \;
</code></pre>
<p>ディレクトリのパーミッションを再帰的に変更する。</p>
<pre class="block">
<code>$ find [ディレクトリパス] -type d -exec chmod 755 {} \;
</code></pre>
<h3 id="grep" class="title">grep</h3>
<p>パターン文字列にマッチした前後の行も表示する。</p>
<pre class="block">
<code>$ grep -C [行数] [パターン文字列] [ファイルパス]
</code></pre>
<p>設定ファイルのコメントと空行以外を表示する。</p>
<pre class="block">
<code>$ grep -v -e '^\s*#' -e '^\s*$' [ファイルパス]
</code></pre>
<p>パターン文字列のリストをファイルから読み込む。</p>
<pre class="block">
<code>$ grep -f [パターン文字列を記載したファイル] [ファイルパス]
</code></pre>
<p>ハイフンから始まる文字列を検索する。</p>
<pre class="block">
<code>$ grep -- [ハイフンから始まるパターン文字列] [ファイルパス]
</code></pre>
<p>psコマンドのヘッダを表示し、grep自身は除いて表示する。</p>
<pre class="block">
<code>$ ps aux | grep -e [U]SER -e [パターン文字列]
</code></pre>
<h3 id="ip" class="title">ip</h3>
<p>指定したIPアドレスに到達するための経路を表示する。</p>
<pre class="block">
<code>$ ip route get [IPアドレス]
</code></pre>
...
<p>相対パスを絶対パスに変換する。</p>
<pre class="block">
<code>$ readlink -f [相対パス]
</code></pre>
<h3 id="sed" class="title">sed</h3>
<p>ファイルの5行目を表示する。</p>
<pre class="block">
<code>$ sed -n 5p [ファイルパス]
</code></pre>
<p>ファイルの10～20行目を表示する。</p>
<pre class="block">
<code>$ sed -n 10,20p [ファイルパス]
</code></pre>
<p>ファイルの5行目を削除する。</p>
<pre class="block">
<code>$ sed -e '5d' [ファイルパス]
</code></pre>
<p>ファイルの10～20行目を削除する。</p>
<pre class="block">
<code>$ sed -e '10,20d' [ファイルパス]
</code></pre>
<p>特定の文字列を含む行を削除する。</p>
<ul>
...
<pre class="block">
<code>$ sed -i -e '/[文字列>/d' [ファイルパス]
</code></pre>
<p>特定の文字列全てを別の文字列に置換する。</p>
<pre class="block">
<code>$ sed -e 's/[置換前の文字列>/[置換後の文字列>/g' [ファイルパス]
</code></pre>
<h3 id="snmpwalk" class="title">snmpwalk</h3>
<p>全MIBを取得する。</p>
<pre class="block">
<code>$ snmpwalk -v 2c -c [コミュニティ名] [IPアドレス]
</code></pre>
<h3 id="ssh" class="title">ssh</h3>
<p>データベースにないホストへの接続時に警告メッセージを表示しない。</p>
<pre class="block">
<code>$ ssh -o StrictHostKeyChecking=no [ユーザ名]@[ホスト名またはIPアドレス]
</code></pre>
<p>ローカルのシェルスクリプトをリモートホストで実行する。</p>
<pre class="block">
<code>$ cat [ファイルパス] | ssh [ユーザ名]@[ホスト名またはIPアドレス] bash
</code></pre>
...
<pre class="block">
<code>$ [コマンド] 2&gt;&amp;1 | tee [オプション] [ファイルパス]
</code></pre>
<h3 id="test" class="title">test</h3>
<pre class="block">
<code>$ test [オプション] [文字列またはファイルパス]
</code></pre>
